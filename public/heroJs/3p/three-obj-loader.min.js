            ...
            var s = new function() {
                var t = {
                    ...
                    addPointGeometry: function(t) {
                        ...
                        for (var e = this.vertices.length, r = 0, i = t.length; r < i; r++) this.addVertexPoint(this.parseVertexIndex(t[r], e))
                    },
                    addLineGeometry: function(t, e) {
                        this.object.geometry.type = "Line";
                        for (var r = this.vertices.length, i = this.uvs.length, s = 0, a = t.length; s < a; s++) this.addVertexLine(this.parseVertexIndex(t[s], r));
                        var n = 0;
                        for (a = e.length; n < a; n++) this.addUVLine(this.parseUVIndex(e[n], i))
                    }
                };
                return t.startObject("", !1), t
            }; - 1 !== i.indexOf("\r\n") && (i = i.replace(/\r\n/g, "\n")), -1 !== i.indexOf("\\\n") && (i = i.replace(/\\\n/g, ""));
            for (var a = i.split("\n"), n = "", o = "", h = [], l = "function" == typeof "".trimLeft, u = 0, c = a.length; u < c; u++)
                if (n = a[u], 0 !== (n = l ? n.trimLeft() : n.trim()).length && "#" !== (o = n.charAt(0)))
                    if ("v" === o) {
                        var p = n.split(/\s+/);
                        switch (p[0]) {
                            case "v":
                ...
                for (var m = n.substr(1).trim().split(/\s+/), f = [], d = 0, v = m.length; d < v; d++) {
                    var g = m[d];
                    if (g.length > 0) {
                        var b = g.split("/");
                        f.push(b)
                ...
                var E = f[0];
                for (d = 1, v = f.length - 1; d < v; d++) {
                    var x = f[d],
                        j = f[d + 1];
                    s.addFace(E[0], x[0], j[0], E[1], x[1], j[1], E[2], x[2], j[2])
                }
            } else if ("l" === o) {
                var y = n.substring(1).trim().split(" "),
                    L = [],
                    H = [];
                if (-1 === n.indexOf("/")) L = y;
                else
                    for (var R = 0, T = y.length; R < T; R++) {
                        var w = y[R].split("/");
                        "" !== w[0] && L.push(w[0]), "" !== w[1] && H.push(w[1])
                    }
                s.addLineGeometry(L, H)
            } else if ("p" === o) {
                var V = n.substr(1).trim().split(" ");
                s.addPointGeometry(V)
            } else if (null !== (h = t.exec(n))) {
                var M = (" " + h[0].substr(1).trim()).substr(1);
                s.startObject(M)
            } else if (r.test(n)) s.object.startMaterial(n.substring(7).trim(), s.materialLibraries);
            else if (e.test(n)) s.materialLibraries.push(n.substring(7).trim());
            else {
                if ("s" !== o) {
                    if ("\0" === n) continue;
                    throw new Error('THREE.OBJLoader: Unexpected line: "' + n + '"')
                }
                if ((h = n.split(" ")).length > 1) {
                    var F = h[1].trim().toLowerCase();
                    s.object.smooth = "0" !== F && "off" !== F
                } else s.object.smooth = !0;
                (J = s.object.currentMaterial()) && (J.smooth = s.object.smooth)
            }
            s.finalize();
            var I = new THREE.Group;
            for (I.materialLibraries = [].concat(s.materialLibraries), u = 0, c = s.objects.length; u < c; u++) {
                var P = s.objects[u],
                    A = P.geometry,
                    z = P.materials,
                    B = "Line" === A.type,
                    C = "Points" === A.type,
                    O = !1;
                if (0 !== A.vertices.length) {
                    var U = new THREE.BufferGeometry;
                    U.addAttribute("position", new THREE.Float32BufferAttribute(A.vertices, 3)), A.normals.length > 0 ? U.addAttribute("normal", new THREE.Float32BufferAttribute(A.normals, 3)) : U.computeVertexNormals(), A.colors.length > 0 && (O = !0, U.addAttribute("color", new THREE.Float32BufferAttribute(A.colors, 3))), A.uvs.length > 0 && U.addAttribute("uv", new THREE.Float32BufferAttribute(A.uvs, 2));
                    for (var N, G = [], S = 0, _ = z.length; S < _; S++) {
                        var D = z[S],
                            J = void 0;
                        if (null !== this.materials)
                            if (J = this.materials.create(D.name), !B || !J || J instanceof THREE.LineBasicMaterial) {
                                if (C && J && !(J instanceof THREE.PointsMaterial)) {
                                    var k = new THREE.PointsMaterial({
                                        size: 10,
                                        sizeAttenuation: !1
                                    });
                                    q.copy(J), J = k
                                ...
                                q.copy(J), q.lights = !1, J = q
                            }
                        J || ((J = B ? new THREE.LineBasicMaterial : C ? new THREE.PointsMaterial({
                            size: 1,
                            sizeAttenuation: !1
                        }) : new THREE.MeshPhongMaterial).name = D.name), J.flatShading = !D.smooth, J.vertexColors = O ? THREE.VertexColors : THREE.NoColors, G.push(J)
                    }
                    if (G.length > 1) {
                        for (S = 0, _ = z.length; S < _; S++) D = z[S], U.addGroup(D.groupStart, D.groupCount, S);
                        N = B ? new THREE.LineSegments(U, G) : C ? new THREE.Points(U, G) : new THREE.Mesh(U, G)
                    } else N = B ? new THREE.LineSegments(U, G[0]) : C ? new THREE.Points(U, G[0]) : new THREE.Mesh(U, G[0]);
                    N.name = P.name, I.add(N)
                }
            }
